...
class AppState {
  ...
  factory AppState._debug() {
    ...
    tt.add(Transaction.initial(
        personID: p.id, type: TransactionType.credit, amount: 7, description: 'Lunch', completed: true));
    ppl.add(p);
    ...
    tt.add(
        Transaction.initial(personID: p.id, type: TransactionType.settleDebt, amount: 4.99, description: 'Some debt'));
    tt.add(Transaction.initial(personID: p.id, type: TransactionType.credit, amount: 9.99, description: 'Some credit'));
    tt.add(Transaction.initial(
        personID: p.id, type: TransactionType.settleCredit, amount: 9.99, description: 'Some credit'));
    ppl.add(p);
  ...
  List<Transaction> getTransactionsOf(Person p) =>
      transactions.where((t) => t.personID == p.id && !t.completed).toList();

  List<Transaction> getCompletedTransactionsOf(Person p) =>
      transactions.where((t) => t.personID == p.id && t.completed).toList();
  ...
  double _sumFold(double value, Transaction t) => t.type.addOrSub(value, t.amount);

  Text getTotalAmountTextTile(Person p, BuildContext context) {
    var amount = getTransactionsOf(p).where((t) => !t.completed).fold(0.0, _sumFold);
    var color = amount == 0 ? Colors.white : (amount.isNegative ? DarkColors.orange : DarkColors.lightGreen);
    return Text(AppLocalizations.of(context).currencyFormatter.format(amount.abs()),
